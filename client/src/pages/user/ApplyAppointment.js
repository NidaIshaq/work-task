...
const ApplyAppointment = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "/api/v1/user/applyAppointment",
        {
          date,
          time,
          doctorId,
          userId: "someUserId", // Add the userId here or get it from localStorage if required
        },
        {
      ...
      setSuccessMessage(
        `Your appointment on ${new Date(
          date
        ).toDateString()} at ${time} has been booked successfully.`
      );
      setShowAlert(true);
      setDate("");
      setTime("");
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen bg-teal-100 py-6">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center max-w-screen-lg w-full mt-2 px-4">
        ...
        <div className="relative max-w-md w-full lg:w-1/2 mt-6 lg:mt-0">
          <img
